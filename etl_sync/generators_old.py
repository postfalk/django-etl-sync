...
class BaseInstanceGenerator(object):
    ...
    def _get_persistence_query(self, model_instance, persistence):
        ...
        """
        # TODO: pure function?
        print(model_instance.__class__, persistence)
        query = Q()
    ...
    def _check_persistence(self, instance, persistence):
        ...
        return self._get_persistence_query(instance, persistence)
        ...
        for (key, lst) in iteritems(rel_inst_dic):
            ...
            try:
                ...
                for i, item in enumerate(lst):
                    ...
                    generator.get_instance(newdic)

    def create_in_db(self, instance):
        """
    ...
    def get_instance(self, dic):
        ...
        if model_instance:
            # print(model_instance, model_instance.__class__)
            if model_instance.pk:
                self.res['exists'] = True
                return model_instance
            qs = self._check_hash(model_instance, self.hashfield)
            count = len(qs)
            if count != 0:
                self.res['exists'] = True
                return qs[0]
            qs = self._check_persistence(model_instance, persistence)
            count = len(qs)
            # print('persistence count', count, model_instance.__class__)
            if count == 0:
                if create:
                    # print('CREATE', model_instance, model_instance.__class__)
                    # print(model_to_dict(model_instance))
                    model_instance = self.create_in_db(model_instance)
                ...
                if update:
                    model_instance = self.update_in_db(model_instance, qs, dic)
            else:
                # for item in qs:
                #    print(model_to_dict(model_instance))
                # print(model_instance, count)
                ...
                return model_instance
            self._assign_related(model_instance, self.related_instances, dic)
            return model_instance
...
class InstanceGenerator(BaseInstanceGenerator):
    ...
    def _prepare_fk(self, field, value):
        return FkInstanceGenerator(field).get_instance(value)
...
class RelInstanceGenerator(InstanceGenerator):
    ...
    def __init__(self, field, **kwargs):
        super(RelInstanceGenerator, self).__init__(None, **kwargs)
...
class FkInstanceGenerator(RelInstanceGenerator):
    ...
    def __init__(self, field, **kwargs):
        super(FkInstanceGenerator, self).__init__(field, **kwargs)
        self.field = field
